...
const finishTest = async () => {
  console.log(
    "НАЧАЛО finishTest. Состояние user.value:",
  ...
  if (totalQuestions > 0) {
    for (const questionId in userAnswers.value) {
      if (
        Object.prototype.hasOwnProperty.call(userAnswers.value, questionId) &&
  ...
  console.log(
    "Тест завершен! Финальные ответы:",
    JSON.parse(JSON.stringify(userAnswers.value))
  );
  console.log(
    `Результат: ${correctCount}/${totalQuestions} (${scorePercentage.toFixed(
      2
    )}%). Оценка: ${finalGrade.value} (${numericGradeValue})`
  );

  let studentIdForGrade;

  if (user.value && user.value.student && user.value.student.length > 0) {
    const studentProfile = user.value.student[0];
    if (studentProfile && studentProfile.id !== undefined) {
      studentIdForGrade = studentProfile.id;
      console.log("Найден studentId для оценки:", studentIdForGrade);
    } else {
      console.error(
        "ID профиля студента не найден в user.value.student[0].",
        studentProfile
      );
    }
  } else {
    console.warn(
      "Профиль студента (user.value.student) отсутствует или пуст. studentId не определен.",
      JSON.parse(JSON.stringify(user.value))
    );
  }

  if (studentIdForGrade !== undefined && numericGradeValue !== undefined) {
    const gradeData = {
    ...
    try {
      console.log("Отправка оценки:", gradeData);
      const response = await http.post("/createGrades", gradeData);
      console.log("Ответ сервера (оценка):", response.data);
      ...
      alert(
        `Тест завершен! ${finalGrade.value}\nНе удалось сохранить оценку. Причина: ${errorMessage}`
      );
    }
  } else {
    let alertMessage = `Тест завершен! ${finalGrade.value}\n`;
    if (studentIdForGrade === undefined) {
      alertMessage += "Не удалось сохранить оценку: ID студента не определен.";
    } else {
      alertMessage +=
        "Не удалось сохранить оценку: числовая оценка не определена.";
    }
    alert(alertMessage);
    console.warn(
      "Оценка не отправлена (отсутствует studentId или numericGradeValue).",
      { studentIdForGrade, numericGradeValue }
    );
  }
...
onMounted(async () => {
  ...
  if (storedUser) {
    try {
      ...
      user.value = parsedUser; // Присваиваем данные user
      console.log(
        "USER LOADED (onMounted):",
        JSON.parse(JSON.stringify(user.value))
      ); // Проверка структуры
    } catch (error) {
  ...
  if (currentTestId) {
    await fetchTestDetails(currentTestId); // Сделал await, если это важно для последующих шагов
    await fetchQuestionsAll(currentTestId); // Сделал await
  } else {
